...
export default function Header() {
  ...
  const menuEl = Object.keys(menu).map((key, index) => {
    if (Array.isArray(menu[key])) {
      const subElements = (
        <ul
          className={`${
            subMenuToDisplay === index ? "translate-y-full" : ""
          } overflow-hidden transition-transform transform duration-500 absolute bg-white border-4 border-brand-500 -mb-4 w-full rounded-b-lg border-t-0 bottom-0 -ml-4 flex flex-col`}
          style={{
            zIndex: -10,
          }}
        >
          {menu[key].map((el) => (
            <li className="hover:text-brand-400" key={key}>
      ...
      return (
        <li
          className="relative w-full group px-4 h-full text-center"
          id={"" + index}
          onClick={() => handleClick(index)}
        >
          <div className="flex items-center hover:text-brand-400">
            {key}{" "}
            <div
              className={`ml-2 transform transition-transform duration-200 ${
                subMenuToDisplay === index ? "rotate-180" : ""
              }`}
            >
              <ChevronDown />
            </div>
    ...
    return (
      <li
        className="relative w-full group px-4 h-full"
        id={"" + index}
        onClick={handleClick}
      >
  ...
  return (
    <header className="relative flex-grow-0 border-b-4 border-brand-500 bg-white w-full z-10">
      <nav className="text-2xl lowercase flex justify-between items-center text-brand-500 py-4 px-4 md:px-24 z-20 bg-white">
